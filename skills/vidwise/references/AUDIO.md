...
落到 `vidwise/public/audio/<chapter-id>/<step-N>.mp3`。运行时
...
| `edge-tts`| —— | 免费无需 key（`pip install edge-tts`）；中文可选多 |

换 / 加 provider 见
[`scripts/tts-providers/README.md`](../templates/scripts/tts-providers/README.md)
（脚手架跑完后路径是 `vidwise/scripts/tts-providers/README.md`）。
...
- 用内置 `edge-tts`（免费）→ 见下方 edge-tts 小节
...
# 换音色 + HD 模型
OPENAI_TTS_MODEL=tts-1-hd VIDWISE_TTS=openai \
...
| `--voice=` / `VIDWISE_TTS_VOICE` | `alloy` | 可选 alloy / echo / fable / onyx / nova / shimmer |

`tts_check` 会检查 curl / jq / `OPENAI_API_KEY` 三件套，缺哪个报哪个。

#### 2.B' 用内置 edge-tts 合成（免费，无需 key）

```bash
pip install edge-tts                  # 或 uv add edge-tts
VIDWISE_TTS=edge-tts npm run synthesize-audio
# 指定中文音色
VIDWISE_TTS_VOICE=zh-CN-XiaoxiaoNeural VIDWISE_TTS=edge-tts npm run synthesize-audio
```

edge-tts 使用 Microsoft Edge 的 TTS 后端，免费且中文音色多。macOS/Linux/Windows 均可用。
详见 `scripts/tts-providers/edge-tts.sh`。
...
```bash
VIDWISE_TTS=elevenlabs npm run synthesize-audio
# 或
...
如果 minimax 和 openai 都没就绪，先试 edge-tts（免费内置）：
...
我可以：
  ...
  3. 换其它 provider
     ...
     再 VIDWISE_TTS=<name> npm run synthesize-audio
...
合成完后可以生成 SRT 字幕文件（需要 `ffprobe`，即 ffmpeg 自带）：

```bash
npm run generate-subtitles   # 输出 subtitles.srt
```

生成的 SRT 按 `audio-segments.json` 顺序，每条对应一个 step 的口播文本，
时间戳从每个 mp3 的实际时长自动计算。可直接导入剪映 / DaVinci Resolve /
Premiere 用作字幕轨。
