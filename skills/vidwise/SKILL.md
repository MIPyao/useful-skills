...
└── vidwise/       # 脚手架产出的 Vite + React + TS 项目
    ├── src/chapters/<NN>-<id>/
    │   ├── <Chapter>.tsx     # 视觉实现
    │   ├── <Chapter>.css
    ...
    │   ├── generate-subtitles.ts   # 从 mp3 时长生成 SRT 字幕
    ...
    │       └── edge-tts.sh         # 免费，无需 key（pip install edge-tts）
...
1. **subAgent**：开一个独立的 reviewer agent，给它"产出文件
   路径 + 对应清单 + 关键上下文"，让它逐项核查并**严格汇报结论**
   （哪几条 pass / 哪几条 fail + 证据 + 改写建议）。
2. **自检（兜底）**：当前 agent 没有上述能力，就自己**严格逐项**核查 —— 不允许目测一遍就放行。
...
| Phase 3 音频合成 | `references/AUDIO.md`（含 narrations.ts → segments.json → 任意 provider 流程，内置 minimax + openai + edge-tts，可选生成 SRT 字幕） | `templates/scripts/tts-providers/README.md`（换 provider / 自带 TTS 时） |
...
对 `script.md` / `outline.md` 执行（优先 subAgent → 自检），
...
```bash
bash <path-to-vidwise>/scripts/scaffold.sh \
  ./vidwise \
  --theme=<用户选的主题 id>

bash <path-to-vidwise>/scripts/scaffold.sh --list-themes
```
...
```bash
rm -rf vidwise/src/chapters/01-example
```

并把 `vidwise/src/registry/chapters.ts` 里 `EXAMPLE_CHAPTER`
的 import 和数组项移除。
...
- **完工自检逐项过**，不达标回去改 —— 按上文「硬性自检协议」执行
  （优先 subAgent → 自检），**改完再向用户汇报本章交付**

### 2.5 大改后 bump STORAGE_KEY

改动 `chapters.ts`（增加 / 删除 / 重排章节，或某章 `narrations.ts`
长度变化）后，**bump** `vidwise/src/hooks/useStepper.ts` 的
...
要不要合成音频做"自动播放录屏"？
  ✓ 合成 → 扫所有章节的 narrations.ts 出 audio-segments.json，
           ...
           内置三个 provider：
              ...
              • edge-tts (免费，无需 key) —— pip install edge-tts
...
VIDWISE_TTS=openai npm run synthesize-audio
# 或用内置 edge-tts (免费，无需 key):
pip install edge-tts
VIDWISE_TTS=edge-tts npm run synthesize-audio
...
合成完此时可选：生成 SRT 字幕文件（需要 ffprobe/ffmpeg）：

```bash
npm run generate-subtitles   # 输出 subtitles.srt
# 或指定输出路径
...
| [`templates/scripts/tts-providers/README.md`](templates/scripts/tts-providers/README.md) | 换 / 加 TTS provider 时 | 三函数契约 + 内置 3 个 (minimax / openai / edge-tts) + 5 种现成代码片段（ElevenLabs / macOS say / Azure / Google） |
| [`references/RECORDING.md`](references/RECORDING.md) | Phase 4 才读 | 录屏工具 + 后期合成 |
| [`scripts/generate-subtitles.ts`](templates/scripts/generate-subtitles.ts) | Phase 3 合成完后可选 | 从 mp3 时长生成 SRT 字幕文件 |
