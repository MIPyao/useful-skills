#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────
# scaffold.sh —— 一键脚手架，创建一个 vidwise 项目。
#
# 用法：
...
#
# 例子：
#   bash <path-to-vidwise>/scripts/scaffold.sh ./vidwise
#   bash <path-to-vidwise>/scripts/scaffold.sh ./talk --theme=paper-press
...
#
# 之后切换主题，覆盖一个文件即可：
#   cp <path-to-vidwise>/themes/<id>/tokens.css \
...
# ── Ensure node is reachable (Windows Git Bash often has npm but not node in PATH) ──
if ! command -v node >/dev/null 2>&1; then
  # npm is guaranteed present (checked above). Resolve node relative to npm.
  NPM_BIN="$(cd "$(dirname "$(command -v npm)")" && pwd)"
  if [ -x "$NPM_BIN/node" ]; then
    export PATH="$NPM_BIN:$PATH"
    echo "▸ (Windows) 通过 npm 路径找到 node：$NPM_BIN/node"
  elif [ -x "$NPM_BIN/node.exe" ]; then
    export PATH="$NPM_BIN:$PATH"
    echo "▸ (Windows) 通过 npm 路径找到 node：$NPM_BIN/node.exe"
  else
    echo "✗ node 不在 PATH 中，也无法通过 npm 定位。请先安装 Node.js。" >&2
    exit 1
  fi
fi
...
cd "$TARGET"
echo "▸ 安装依赖（可能要等一会）..."
npm install 2>install-err.log; install_ret=$?; [ $install_ret -ne 0 ] && { echo "✗ npm install 失败，见
  install-err.log" >&2; cat install-err.log >&2; rm -f install-err.log; exit 1; }; rm -f install-err.log

echo "▸ 安装 tsx（用于 extract-narrations 脚本）..."
 npm install --save-dev tsx 2>tsx-err.log; tsx_ret=$?; [ $tsx_ret -ne 0 ] && { echo "✗ tsx 安装失败，见 tsx-err.log"
  >&2; cat tsx-err.log >&2; rm -f tsx-err.log; exit 1; }; rm -f tsx-err.log
...
cp "$TEMPLATES/scripts/generate-subtitles.ts"   scripts/generate-subtitles.ts
...
p.scripts = Object.assign({}, p.scripts, {
  ...
  "generate-subtitles": "tsx scripts/generate-subtitles.ts",
});
...
✓ 完成。下一步：
  ...
  1. cd $TARGET
  2. npm run dev      # 默认 http://localhost:5173（被占会自动换端口）
...
录制：
  ...
  • 手动模式：直接打开 http://localhost:5173（点击 / 方向键推进）
...
音频合成（可选，录制前做）：
  ...
  npm run synthesize-audio      # 默认 minimax provider 合成 → public/audio/<id>/<step>.mp3
                                # 换 provider：VIDWISE_TTS=<name> npm run synthesize-audio
