...
PROVIDER="${VIDWISE_TTS:-minimax}"
VOICE="${VIDWISE_TTS_VOICE:-}"
FORCE=false
...
# jq detection — also find jq.exe on Windows / WSL
_JQ=jq
...
# ── jq path compat: WSL /mnt/X → X:/ for Windows jq.exe ─────────────
_JQ_SEGMENTS="$SEGMENTS"
if [[ "$_JQ_SEGMENTS" == /mnt/?/* ]]; then
  _drive="${_JQ_SEGMENTS:5:1}"
  _JQ_SEGMENTS="${_drive^^}:${_JQ_SEGMENTS:6}"
fi

# ── Main loop ─────────────────────────────────────────────────────────
# Use mapfile + for-loop (not while-read process substitution) to avoid
# issues with set -u + WSL process substitution exiting early.
total=$($_JQ 'length' "$_JQ_SEGMENTS" | tr -d '\r')
i=0
synthesized=0
skipped=0
failed=0

mapfile -t segments < <($_JQ -c '.[]' "$_JQ_SEGMENTS" | tr -d '\r')

for row in "${segments[@]}"; do
  i=$((i + 1))
  chapter=$(echo "$row" | $_JQ -r '.chapter' | tr -d '\r')
  step=$(echo "$row" | $_JQ -r '.step' | tr -d '\r')
  text=$(echo "$row" | $_JQ -r '.text' | tr -d '\r')
