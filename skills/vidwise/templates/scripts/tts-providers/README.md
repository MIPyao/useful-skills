...
`tts_install_help`。runner 根据 `VIDWISE_TTS` 环境变量加载对应文件。
...
VIDWISE_TTS_VOICE=alloy npm run synthesize-audio
...
| `edge-tts.sh`  | Microsoft Edge TTS      | 无需 key，免费              | `pip install edge-tts`；中文可选多 |

其它后端的代码片段在下面，复制到 `tts-providers/<name>.sh` 即可启用。
...
4. `VIDWISE_TTS=<name> npm run synthesize-audio`
