...
/**
 * Estimate spoken duration. Chinese ≈ 4 char/s → 250ms/char.
 * English ≈ 2.5 word/s → 150ms/word.
 * Used as Auto-mode fallback ONLY when audio is missing / fails / empty.
 */
function estimateMs(text: string): number {
  if (!text) return 1500;
  const hasChinese = /[一-鿿]/.test(text);
  if (hasChinese) return Math.max(1500, text.length * 250);
  const words = text.split(/\s+/).filter(Boolean).length;
  return Math.max(1500, words * 150);
}
